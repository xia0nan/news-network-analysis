...
@retry(wait=wait_fixed(6) + wait_random(0, 2), stop=stop_after_attempt(7))
def resquest_resp(title, body):
    method="POST"
    payload = f"<Document>\n<Title> {title} </Title>\n<Body> {body} </Body>\n</Document>"
    headers = {
...
for index, row in news.iterrows():
    ...
    response = resquest_resp(title, body)
    ...
    # check if permid already in the temp_df
    # exception: person use same permid
    # N.B. Keep permid consistent in string format
    # N.B. Keep permid for company consitent with 10 digits
    orig_permids = eval(row['permids'])
    # print('temp_df', temp_df)
    # print('orig_permids', orig_permids)
    temp_list=[]

    # convert number col to string before compare
    temp_df['permid'] = temp_df['permid'].apply(str)

    for id in orig_permids:
        # print('original permid tagged', id)
        if id not in temp_df['permid'].tolist():
            print('not in')
            temp_list.append([news_id, '_TBL', 'Company', id])

    if len(temp_list) > 1:
        print('extra')
        temp_df2 = pd.DataFrame(temp_list, columns=['news_id', 'name','dtype','permid'])
        frames.append(temp_df2)

    # sleep for 1s
    time.sleep(1)

# # manipulate person for testing
# fake_list = [['test_name_id', 'Trump', 'Person', 'Trump'],
#              ['test_name_id_2', 'Trump', 'Person', 'Trump'],
#              ['test_name_id', 'Obama', 'Person', 'Obama'],
#              ['test_name_id_2', 'Obama', 'Person', 'Obama']]
# frames.append(pd.DataFrame(fake_list, columns=['news_id', 'name','dtype','permid']))
...
result.to_csv("12_apiout.csv", index=False)

# save as pickle
with open('12_api_out.pkl', 'wb') as f:
